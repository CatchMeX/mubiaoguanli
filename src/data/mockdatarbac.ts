...
// 权限冲突处理规则
interface PermissionConflictRule {
  id: string;
  name: string;
  description: string;
...
const conflictRules: PermissionConflictRule[] = [
  {
    id: 'explicit-deny',
    name: '显式拒绝优先',
...
// 检查用户是否有特定权限
const hasPermission = (userId: string, permissionCode: string): boolean => {
  const userPermissions = getUserPermissions(userId);
  return userPermissions.includes(permissionCode);
};

// 获取角色关联的用户
const getRoleUsers = (roleId: string): User[] => {
