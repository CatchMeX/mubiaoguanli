...
interface FinancialMatterPDFExportProps {
  matter: FinancialMatter;
  onExport: () => Promise<{
...
const FinancialMatterPDFExport: React.FC<FinancialMatterPDFExportProps> = ({ matter, onExport }) => {
          ...
          <div ref={contentRef} style={{ 
            ...
            <div style={{ marginBottom: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span>事项ID：{matter.id}</span>
                <span>提交时间：{new Date(matter.created_at).toLocaleString('zh-CN')}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span>申请人：{matter.applicant?.name || ''}</span>
                <span>申请人部门：{matter.department?.name || ''}</span>
              </div>
              ...
              <div style={{ marginBottom: '10px' }}>
                <span>事项简述：{matter.matter_description}</span>
              </div>
              <div style={{ marginBottom: '10px' }}>
                <span>申请金额：¥{matter.amount.toFixed(2)}（{formatAmountToChinese(matter.amount)}）</span>
              </div>
              <div style={{ marginBottom: '10px' }}>
                <span>是否总商维度：{matter.is_corporate_dimension ? '是' : '否'}</span>
              </div>
              {matter.team && (
                <div style={{ marginBottom: '10px' }}>
                  <span>费用归属团队：{matter.team.name}</span>
                </div>
              )}
            </div>

            {/* Allocation Details - 分摊明细 */}
            {matter.is_corporate_dimension && exportData.allocations && exportData.allocations.length > 0 && (
              <div style={{ marginBottom: '20px' }}>
                <h3 style={{ fontSize: '16px', fontWeight: 'bold', marginBottom: '10px' }}>分摊明细</h3>
                <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #000' }}>
                  <thead>
                    <tr style={{ backgroundColor: '#f0f0f0' }}>
                      <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '30%' }}>团队名称</th>
                      <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '20%' }}>分摊比例</th>
                      <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '30%' }}>分摊金额</th>
                      <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '20%' }}>备注</th>
                    </tr>
                  </thead>
                  <tbody>
            ...
            <div style={{ marginBottom: '20px', fontSize: '12px', color: '#666' }}>
              <p style={{ margin: '0' }}>
                ▲ 根据《财务管理制度》之规定：财务事项申请需经过相关部门审批，总商维度事项需进行团队分摊。
              </p>
            </div>
...
export default FinancialMatterPDFExport; 