...
interface BusinessTripPDFExportProps {
  reimbursement: BusinessTripReimbursement;
  onExport: () => Promise<{
...
const BusinessTripPDFExport: React.FC<BusinessTripPDFExportProps> = ({ reimbursement, onExport }) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      setTimeout(async () => {
        ...
        pdf.save(`差旅费报销_${reimbursement.request_number}.pdf`);
        setIsLoading(false);
      }, 100);
    } catch (error) {
  ...
  // 计算总金额的函数
  const calculateTotalAmount = (details: BusinessTripExpenseDetail[] = []) => {
    return details.reduce((sum, detail) => sum + (detail.total_fee || 0), 0);
  };
  ...
  const totalAmount = exportData ? calculateTotalAmount(exportData.expenseDetails) : 0;
          ...
          <div ref={contentRef} style={{ 
            ...
            <div style={{ marginBottom: '20px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span>申请单号：{reimbursement.request_number}</span>
                <span>提交时间：{new Date(reimbursement.created_at).toLocaleString('zh-CN')}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span>申请人：{reimbursement.applicant?.name || ''}</span>
                <span>申请人部门：{reimbursement.department?.name || ''}</span>
              </div>
              ...
              <div style={{ marginBottom: '10px' }}>
                <span>报销事由：{reimbursement.expense_reason}</span>
              </div>
            </div>

            {/* Reimbursement Details - 按行展示 */}
            <div style={{ marginBottom: '20px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 'bold', marginBottom: '10px' }}>报销明细</h3>
              <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #000' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f0f0f0' }}>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '15%' }}>住宿费</th>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '15%' }}>城际交通费</th>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '15%' }}>市内交通费</th>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '15%' }}>其他费用</th>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '15%' }}>费用小计</th>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '25%' }}>说明</th>
                  </tr>
                </thead>
                <tbody>
                  {exportData.expenseDetails?.map((detail, index) => (
                    <tr key={index}>
                      <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'right' }}>
                        {detail.accommodation_fee?.toFixed(2) || '0.00'}
                      </td>
                      <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'right' }}>
                        {detail.intercity_transport_fee?.toFixed(2) || '0.00'}
                      </td>
                      <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'right' }}>
                        {detail.local_transport_fee?.toFixed(2) || '0.00'}
                      </td>
                      <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'right' }}>
                        {detail.other_fees?.toFixed(2) || '0.00'}
                      </td>
                      <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'right', fontWeight: 'bold' }}>
                        {detail.total_fee?.toFixed(2) || '0.00'}
                      </td>
                  ...
                  <tr style={{ backgroundColor: '#f0f0f0', fontWeight: 'bold' }}>
                    ...
                    <td style={{ border: '1px solid #000', padding: '8px' }}>
                      {formatAmountToChinese(totalAmount)}
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            {/* Bank Information - 按行展示 */}
            <div style={{ marginBottom: '20px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 'bold', marginBottom: '10px' }}>银行信息</h3>
              <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #000' }}>
                <thead>
                  <tr style={{ backgroundColor: '#f0f0f0' }}>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center' }}>收款人开户名</th>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center' }}>银行账号</th>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center' }}>开户银行</th>
                    <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center' }}>支付金额</th>
                  </tr>
                </thead>
                <tbody>
                  {exportData.bankAccounts?.map((account, index) => (
                    <tr key={index}>
                      <td style={{ border: '1px solid #000', padding: '8px' }}>{account.payee_account_name}</td>
                      ...
                      <td style={{ border: '1px solid #000', padding: '8px', textAlign: 'right' }}>
                        {account.payment_amount?.toFixed(2)}元（{formatAmountToChinese(account.payment_amount || 0)}）
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div style={{ marginBottom: '20px', fontSize: '12px', color: '#666' }}>
              <p style={{ margin: '0' }}>
                ▲ 根据《费用管理制度》之规定：当月25日前发生的费用应当在当月28前报销完毕（跨月出差的差旅费除外），跨月报销费用的将按照实际发生费用金额的95%进行核报费用。
              </p>
            </div>

            {/* Approval Workflow - 去掉状态，正确显示审批节点 */}
...
export default BusinessTripPDFExport; 