...
interface ExpenseReimbursementPDFExportProps {
...
const ExpenseReimbursementPDFExport: React.FC<ExpenseReimbursementPDFExportProps> = ({ reimbursement, onExport }) => {
  ...
  const generatePDF = async () => {
    try {
      ...
      setTimeout(async () => {
        ...
        pdf.save(`费用报销_${reimbursement.request_number}.pdf`);
        setIsLoading(false);
      }, 100);
    } catch (error) {
        ...
        <div style={{ position: 'absolute', left: '-9999px', top: '-9999px' }}>
          <div ref={contentRef} style={{ 
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ marginBottom: '10px' }}>
                <span>报销类别：{reimbursement.expense_category === 'expense_reimbursement' ? '费用报销' : '冲销借款'}</span>
              </div>
              <div style={{ marginBottom: '10px' }}>
                <span>报销事由：{reimbursement.expense_reason}</span>
              </div>
              <div style={{ marginBottom: '10px' }}>
                <span>报销金额：¥{reimbursement.total_amount.toFixed(2)}（{formatAmountToChinese(reimbursement.total_amount)}）</span>
              </div>
            </div>

            {/* Allocation Details - 分摊明细 */}
            {exportData.allocations && exportData.allocations.length > 0 && (
              <div style={{ marginBottom: '20px' }}>
                <h3 style={{ fontSize: '16px', fontWeight: 'bold', marginBottom: '10px' }}>分摊明细</h3>
                <table style={{ width: '100%', borderCollapse: 'collapse', border: '1px solid #000' }}>
                  <thead>
                    <tr style={{ backgroundColor: '#f0f0f0' }}>
                      <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '40%' }}>团队名称</th>
                      <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '30%' }}>分摊比例</th>
                      <th style={{ border: '1px solid #000', padding: '8px', textAlign: 'center', width: '30%' }}>分摊金额</th>
                    </tr>
                  </thead>
                  <tbody>
                    {exportData.allocations.map((allocation, index) => (
                      <tr key={index}>
            ...
            <div style={{ marginBottom: '20px', fontSize: '12px', color: '#666' }}>
              <p style={{ margin: '0' }}>
                ▲ 根据《费用管理制度》之规定：当月25日前发生的费用应当在当月28前报销完毕，跨月报销费用的将按照实际发生费用金额的95%进行核报费用。
              </p>
            </div>
...
export default ExpenseReimbursementPDFExport; 