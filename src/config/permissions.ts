    ...
    children: [
      {
        ...
        children: [
          ...
          },
          {
            ...
            name: '拆分到部门',
            code: 'SPLIT_YEARLY_GOAL',
            type: 'button',
            description: '将年度目标拆分到部门月度目标'
          }
        ]
      },
      {
...
// 根据权限代码获取权限节点
function getPermissionByCode(code: string): PermissionNode | null {
...
// 获取指定类型的所有权限
function getPermissionsByType(type: PermissionType): PermissionNode[] {
...
// 检查权限是否包含子权限
function hasChildren(permission: PermissionNode): boolean {
  return !!(permission.children && permission.children.length > 0);
}

// 获取权限的完整路径（从根到当前节点）
function getPermissionPath(permission: PermissionNode): string[] {
...
// 构建完整的权限树，包含父级引用
function buildCompletePermissionTree(): PermissionNode[] {
...
// 导出完整的权限树
const COMPLETE_FUNCTION_PERMISSIONS = buildCompletePermissionTree();
