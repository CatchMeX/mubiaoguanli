...
const TeamMonthlyGoals = () => {
  ...
  const loadData = async () => {
    try {
      ...
      const [teamGoalsResponse, companyGoalsResponse, departmentsResponse, usersResponse, unitsResponse] = await Promise.all([
  ...
  const calculateTeamProgress = (goal: any) => {
    if (!goal.personalGoals || goal.personalGoals.length === 0) return 0;
    
    // 计算团队总实际收入
    const totalActualRevenue = goal.personalGoals.reduce((sum: number, pg: any) => 
      sum + calculateActualRevenue(pg), 0);
    
    // 基于总实际收入与团队目标值的比例计算进度
    return Math.round((totalActualRevenue / goal.target_value) * 100);
  };

  const calculateTeamActualRevenue = (goal: any) => {
    if (!goal.personalGoals || goal.personalGoals.length === 0) return 0;
    return goal.personalGoals.reduce((sum: number, pg: any) => sum + calculateActualRevenue(pg), 0);
  };

  // 切换个人目标分配的展开/收起状态
  const togglePersonalGoals = (goalId: string) => {
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">
                {teamMonthlyGoals.length > 0 
                  ? Math.round(teamMonthlyGoals.reduce((sum, goal) => sum + calculateTeamProgress(goal), 0) / teamMonthlyGoals.length)
                  : 0}%
              </div>
            </CardContent>
          </Card>
                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-4 mt-2 text-sm text-muted-foreground">
                      <div className="flex items-center space-x-1">
                        ...
                        <span>目标值: {goal.target_value.toLocaleString()} {goal.unit?.name || goal.company_yearly_goal?.unit?.name || ''}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <Calendar className="h-4 w-4" />
                        <span>关联: {goal.company_yearly_goal ? `${goal.company_yearly_goal.year}年${goal.company_yearly_goal.title}` : '无'}</span>
                        {goal.title && (
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span>部门: {goal.department?.name}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm text-foreground">{calculateTeamProgress(goal)}%</span>
                    </div>
                    <Progress value={Math.min(calculateTeamProgress(goal), 100)} className="h-4" />
                    <div className="flex items-center justify-between mt-2 text-sm text-muted-foreground">
                      <span>实际收入: {calculateTeamActualRevenue(goal).toLocaleString()} {goal.unit?.name || goal.company_yearly_goal?.unit?.name || ''}</span>
                      <span>目标值: {goal.target_value.toLocaleString()} {goal.unit?.name || goal.company_yearly_goal?.unit?.name || ''}</span>
                    </div>
                  </div>

                  {goal.personalGoals && goal.personalGoals.length > 0 && (
                    <div>
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="text-sm font-medium text-foreground">个人目标分配</h4>
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => togglePersonalGoals(goal.id)}
                      ...
                      {expandedGoals.has(goal.id) && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                          {goal.personalGoals.map((personalGoal) => (
                            <Card key={personalGoal.id} className="bg-accent border-border theme-transition">
                              <CardContent className="pt-4">
                                <div className="text-center">
                                  <div className="text-lg font-bold text-foreground">{personalGoal.user?.name}</div>
                                  <div className="text-sm text-muted-foreground mb-2">
                                    {personalGoal.target_value.toLocaleString()} {personalGoal.unit?.name || personalGoal.team_monthly_goal?.unit?.name || ''}
                                  </div>
                                  {personalGoal.team_goal_deleted && (
                                  ...
                                  {personalGoal.remark && (
                                    <div className="text-xs text-muted-foreground mt-1">
                                      备注: {personalGoal.remark}
                                    </div>
                                  )}
                                  <Progress value={Math.min(calculateProgress(personalGoal), 100)} className="h-4 mb-2" />
                                  <div className="text-sm text-muted-foreground">
                                    进度: {calculateProgress(personalGoal)}%
                                  </div>
                                </div>
                              </CardContent>
                            </Card>
