...
const CompanyYearlyGoals = () => {
    ...
    goal.team_monthly_goals
      ...
      .forEach(teamGoal => {
        if (teamGoal.personalGoals) {
          // 遍历每个团队目标下的个人目标，过滤掉关联的团队目标已删除的个人目标
  ...
  const calculateTeamGoalProgress = (teamGoal: any): number => {
    ...
    const totalActualValue = calculateTeamGoalActualValue(teamGoal);
    
    // 用部门的实际完成值除以部门的目标值来计算进度
    return teamGoal.target_value > 0 ? Math.round((totalActualValue / teamGoal.target_value) * 100) : 0;
  };
  ...
  const calculateTeamGoalActualValue = (teamGoal: any): number => {
    ...
    // 过滤掉关联的团队目标已删除的个人目标
    const activePersonalGoals = teamGoal.personalGoals.filter((personalGoal: any) => !personalGoal.team_goal_deleted);
            ...
            <Table>
              ...
              <TableBody>
                {goals
                  ...
                  const hasTeamGoals = goal.team_monthly_goals && 
                    goal.team_monthly_goals.filter(teamGoal => !teamGoal.deleted_at).length > 0;
                              ...
                              <>
                                <TableRow key={`team-${teamGoal.id}`} className="border-border bg-muted/30 hover:bg-muted/40 transition-colors">
                                  ...
                                  <TableCell className="text-foreground text-sm">
                                    {(() => {
                                      const totalActualValue = calculateTeamGoalActualValue(teamGoal);
                                      return `${totalActualValue.toLocaleString()} / ${teamGoal.target_value.toLocaleString()} ${teamGoal.unit_id ? units.find(u => u.id === teamGoal.unit_id)?.name : ''}`;
                                    })()}
                                  </TableCell>
                                  <TableCell>
                                    <div className="space-y-1">
