...
const PersonalMonthlyGoals = () => {
  ...
  const getFilteredUsers = () => {
    ...
    const filteredUsers = users.filter(user => {
      ...
      // 检查用户的主要部门
      if (user.primaryDepartment && user.primaryDepartment.id === selectedTeamGoal.department_id) {
        console.log(`用户 ${user.name} 的主要部门匹配:`, user.primaryDepartment.id);
        return true;
      }
      
      // 检查用户的部门关联
      if (user.departments && Array.isArray(user.departments)) {
        const hasMatchingDept = user.departments.some(dept => dept.id === selectedTeamGoal.department_id);
        if (hasMatchingDept) {
          console.log(`用户 ${user.name} 的部门关联匹配:`, user.departments);
        }
        return hasMatchingDept;
      }
      
      // 检查user_departments结构
      if (user.user_departments && Array.isArray(user.user_departments)) {
        const hasMatchingDept = user.user_departments.some(ud => 
          ud.departments && ud.departments.id === selectedTeamGoal.department_id
        );
        if (hasMatchingDept) {
          console.log(`用户 ${user.name} 的user_departments匹配:`, user.user_departments);
        }
        return hasMatchingDept;
      }
  ...
  const getFilteredUsersForEdit = () => {
    ...
    return users.filter(user => {
      ...
      // 检查用户的部门关联
      if (user.departments && Array.isArray(user.departments)) {
        return user.departments.some(dept => dept.id === selectedTeamGoal.department_id);
      }
      
      // 检查user_departments结构
      if (user.user_departments && Array.isArray(user.user_departments)) {
        return user.user_departments.some(ud => 
          ud.departments && ud.departments.id === selectedTeamGoal.department_id
        );
      }
      ...
      <div className="space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => {
                      const selectedTeamGoal = teamMonthlyGoals.find(goal => goal.id === value);
                      setFormData({
                        ...formData, 
                  ...
                  >
                    ...
                    <SelectContent className="bg-popover border-border">
                      {teamMonthlyGoals.map(goal => (
                        <SelectItem key={goal.id} value={goal.id}>
                          {goal.department?.name} - {goal.year}年{getMonthName(goal.month)}
                        </SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card className="bg-card border-border theme-transition">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">个人目标数</CardTitle>
              <Target className="h-4 w-4 text-primary" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-foreground">{personalMonthlyGoals.length}</div>
            </CardContent>
          </Card>
          <Card className="bg-card border-border theme-transition">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <CardContent>
              <div className="text-2xl font-bold text-foreground">
                {personalMonthlyGoals.filter(goal => calculateProgress(goal) < 100).length}
              </div>
            </CardContent>
          </Card>
          <Card className="bg-card border-border theme-transition">
          ...
          <Card className="bg-card border-border theme-transition">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-muted-foreground">平均进度</CardTitle>
              <FileText className="h-4 w-4 text-yellow-500" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-foreground">
                {personalMonthlyGoals.length > 0 
                  ? Math.round(personalMonthlyGoals.reduce((sum, goal) => sum + calculateProgress(goal), 0) / personalMonthlyGoals.length)
                  : 0}%
              </div>
            </CardContent>
          </Card>
        ...
        <div className="grid gap-6">
          {personalMonthlyGoals
            ...
            .map((goal) => (
            <Card key={goal.id} className="bg-card border-border theme-transition">
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-foreground">
                      {goal.user?.name} - {goal.year}年{getMonthName(goal.month)}目标
                    </CardTitle>
                    <div className="flex items-center space-x-4 mt-2 text-sm text-muted-foreground">
                      <div className="flex items-center space-x-1">
                        <Target className="h-4 w-4" />
                        <span>目标值: {goal.target_value.toLocaleString()} {goal.unit?.name || '元'}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <DollarSign className="h-4 w-4" />
                        <span>实际收入: {calculateActualRevenue(goal).toLocaleString()} {goal.unit?.name || '元'}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                        <Target className="h-4 w-4" />
                        <span>关联部门目标: {
                          ...
                          ) : goal.team_monthly_goal_id && goal.team_monthly_goal ? (
                            `${goal.team_monthly_goal.department?.name} - ${goal.team_monthly_goal.year}年${getMonthName(goal.team_monthly_goal.month)}目标`
                          ) : (
                            '无'
                          )
                        }</span>
                      ...
                      <div className="flex items-center space-x-1">
                        ...
                        <span>创建人: {goal.creator?.name || '未知'}</span>
                      </div>
                      <div className="flex items-center space-x-1">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <DeleteButton
                      ...
                      itemName={`${goal.user?.name} - ${goal.year}年${getMonthName(goal.month)}目标`}
                      title="删除个人月度目标"
                      description={`确定要删除"${goal.user?.name} - ${goal.year}年${getMonthName(goal.month)}目标"吗？删除后相关的日报记录也将被移除。`}
                      variant="outline"
                      size="sm"
                    />
                  </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                      ...
                      <span className="text-sm text-foreground">{calculateProgress(goal)}%</span>
                    </div>
                    <Progress value={Math.min(calculateProgress(goal), 100)} className="h-4" />
                  </div>

                  {goal.dailyReports && goal.dailyReports.length > 0 && (
                    <div>
                      <h4 className="text-sm font-medium text-foreground mb-3">最近日报</h4>
                      <div className="bg-accent rounded-lg p-4 theme-transition">
                        <Table>
                          <TableHeader>
                          ...
                          <TableBody>
                            {goal.dailyReports.slice(-3).map((report) => (
                        ...
                        {goal.dailyReports.length > 3 && (
                          <div className="text-center mt-2">
                            <Button 
                              variant="ghost" 
                              size="sm" 
                              className="text-blue-500 hover:bg-accent"
                              onClick={() => openReportDialog(goal)}
                            >
                              查看全部 {goal.dailyReports.length} 条日报
                            </Button>
                          </div>
                        )}
                      </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="border-b border-border pb-3">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-muted-foreground">
                        <div>成员: {selectedGoal.user?.name}</div>
                    ...
                    <div>
                      ...
                      {selectedGoal.team_monthly_goal ? (
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-muted-foreground">
                        ...
                        <div className="text-sm text-muted-foreground">
                          {selectedGoal.team_goal_deleted ? (
                            <span className="text-red-500">关联目标已删除</span>
                          ) : (
                            '无关联部门目标'
                          )}
                        </div>
                ...
                <>
                  ...
                  <div>
                    ...
                    <p className="text-foreground">
                      {selectedGoal.team_monthly_goal 
                        ? `${selectedGoal.team_monthly_goal.department?.name} - ${selectedGoal.team_monthly_goal.year}年${getMonthName(selectedGoal.team_monthly_goal.month)}目标`
                        : '无'
                      }
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-foreground">{selectedGoal.user?.name} - {selectedGoal.user?.position?.name}</p>
                  </div>
                  <div>
