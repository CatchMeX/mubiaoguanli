...
interface CompanyYearlyGoalCardProps {
  ...
  onSplitToTeam: (goal: CompanyYearlyGoal) => void;
  onEdit: (goal: CompanyYearlyGoal) => void;
  onDelete: (goal: CompanyYearlyGoal) => void;
}
            ...
            <div className="space-y-1">
              ...
              {goal.description && (
                <div className="text-sm text-muted-foreground mt-2">
                  <span className="font-medium">目标描述:</span> {goal.description}
                </div>
              )}
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="flex items-center space-x-2">
                ...
                <DeleteButton
                  ...
                  itemName={`${goal.year}年的年度目标"${goal.title || '年度目标'}"`}
                  variant="outline"
                  size="sm"
                />
                <Button
...
interface TeamMonthlyGoalCardProps {
  ...
  progress: number;
  actualRevenue: number;
  onSplitToPersonal: (goal: TeamMonthlyGoal) => void;
}
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <Button
                ...
                onClick={() => onSplitToPersonal(goal)}
              >
                <Split className="mr-1 h-3 w-3" />
                拆分到个人
              </Button>
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        <Card className="bg-card border-border theme-transition">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">{filteredYearlyGoals.length}</div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border theme-transition">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">
              {filteredYearlyGoals.filter(goal => calculateYearlyProgress(goal) < 100).length}
            </div>
          </CardContent>
        </Card>
        <Card className="bg-card border-border theme-transition">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-foreground">
              {filteredYearlyGoals.length > 0 
                ? Math.round(filteredYearlyGoals.reduce((sum, g) => sum + calculateYearlyProgress(g), 0) / filteredYearlyGoals.length)
                : 0
              }%
            </div>
          </CardContent>
          ...
          return (
            <div key={yearlyGoal.id} className="space-y-4">
              ...
              <CompanyYearlyGoalCard
                ...
                onSplitToTeam={handleSplitToTeam}
                    ...
                    return (
                      <div key={monthlyGoal.id} className="space-y-2">
                        <TeamMonthlyGoalCard
                          ...
                          onSplitToPersonal={handleSplitToPersonal}
                        />
