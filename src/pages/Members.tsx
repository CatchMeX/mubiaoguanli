...
import { roles, getUserRoles, userRoles } from '@/data/mockdatarbac';
...
const Members = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // 使用带关联数据的API
      const [usersData, departmentsData, positionsData, jobLevelsData] = await Promise.all([
  ...
  const handleManageRoles = (member: any) => {
    setSelectedMember(member);
    const memberRoles = getUserRolesList(member.id);
  ...
  const handleSaveRoles = async () => {
    ...
    try {
      // 这里需要实现用户角色分配的API调用
      // await userAPI.updateRoles(selectedMember.id, selectedRoles);
      console.log('角色分配功能待实现:', {
            ...
            <Table>
              ...
              <TableBody>
                {filteredUsers.map((user) => {
                  const userRolesList = getUserRolesList(user.id);
                  return (
                      ...
                      <TableCell>
                        <div className="flex flex-wrap gap-1">
                          {userRolesList.length > 0 ? (
                            userRolesList.slice(0, 2).map(role => (
                              <div key={role.id}>
                                {getRoleBadge(role)}
                              </div>
                            ))
